...
public class SpinnerGoodsType extends RelativeLayout {
    // 返回按钮控件
    private Spinner mSp;
    // 标题Tv
    ...
    private String Id="";
    private String Name="";
    private String T="交货方式：";     //15


    public SpinnerGoodsType(Context context, AttributeSet attributeSet) {
...
//                    container.addAll(dBean.purchaseMethod);
                    ...
                    if (dBean.purchaseMethod.size()>0 && container.size()<=0){
                        ...
                        for (int i=0;i<dBean.purchaseMethod.size();i++){
                            if (dBean.purchaseMethod.get(i).FTypeID.equals("32")){
...
//        else {
            ...
            List<PurchaseMethod> purchaseMethods = employeeDao.queryBuilder().
                    where(PurchaseMethodDao.Properties.FTypeID.eq("32"))
