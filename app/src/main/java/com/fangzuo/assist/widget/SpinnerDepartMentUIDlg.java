...
public class SpinnerDepartMentUIDlg extends RelativeLayout {
    // 返回按钮控件
    private Spinner mSp;
    // 标题Tv
    ...
    public SpinnerDepartMentUIDlg(final Context context, AttributeSet attributeSet) {
...
//                }
    ...
    private List<Department> getLocData() {
...
//        autoOrg = org==null?"":org.FOrgID;
        mTitleTv.setText("");
        final List<Department> listTemp = getLocData();
        ...
        if (share.getIsOL()) {
            ...
            App.getRService().doIOAction(WebApi.DOWNLOADDATA, json, new MySubscribe<CommonResponse>() {
                ...
                public void onNext(CommonResponse cBean) {
                    ...
                    DepartmentDao payTypeDao = daoSession.getDepartmentDao();
                    payTypeDao.deleteAll();
                    payTypeDao.insertOrReplaceInTx(dBean.department);
                    payTypeDao.detachAll();
                    if (dBean.department.size()>0 && container.size()<=0){
                        dealAuto(dBean.department,false);
                    }
                }
