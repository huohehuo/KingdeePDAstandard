...
public class SpinnerSaleMethodForSaleOrder extends RelativeLayout {
    // 返回按钮控件
    private Spinner mSp;
    // 标题Tv
    ...
    private String Id="";
    private String Name="";
    private String T="销售方式  ：";     //15  for销售订单


    public SpinnerSaleMethodForSaleOrder(Context context, AttributeSet attributeSet) {
                    ...
                    if (dBean.purchaseMethod.size()>0 && container.size()<=0){
                        ...
                        for (int i=0;i<dBean.purchaseMethod.size();i++){
                            if (dBean.purchaseMethod.get(i).FTypeID.contains("101")
                                    && !dBean.purchaseMethod.get(i).FNumber.equals("06")){
...
//        else {
            ...
            List<PurchaseMethod> purchaseMethods = employeeDao.queryBuilder().
                    where(PurchaseMethodDao.Properties.FTypeID.eq("101"),
                            PurchaseMethodDao.Properties.FNumber.notEq("06"))
