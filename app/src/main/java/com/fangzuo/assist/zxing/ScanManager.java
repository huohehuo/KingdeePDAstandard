package com.fangzuo.assist.zxing;
...
public class ScanManager extends CaptureManager {
    ...
    public ScanManager(Activity activity, DecoratedBarcodeView barcodeView) {
    ...
    protected void returnResult(BarcodeResult rawResult) {
        if (rawResult.getResult().getText().contains("fangzuokeji")){
            EventBusUtil.sendEvent(new ClassEvent(EventBusInfoCode.ScanResult,rawResult));
            closeAndFinish();
        }else{
            EventBusUtil.sendEvent(new ClassEvent(EventBusInfoCode.ScanResult,rawResult));
            closeAndFinish();
...
//        closeAndFinish();
    ...
    public static void setCameraPermissionReqCode(int cameraPermissionReqCode) {
        ScanManager.cameraPermissionReqCode = cameraPermissionReqCode;
    }
}
